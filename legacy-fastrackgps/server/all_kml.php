...
<kml xmlns="http://earth.google.com/kml/2.1">
  <Document>
	...
	<?php
	$res1 = mysql_query("SELECT imei, cor_grafico FROM bem WHERE cliente = '$cliente' AND activated = 'S'");
    ...
    <Style id="highlightPlacemarkGreen<?php echo $imei; ?>">
      <IconStyle>
        <Icon>
...
$res2 = mysql_query("SELECT id, imei, name, identificacao FROM bem WHERE cliente = $cliente AND activated = 'S'");
while($data2 = mysql_fetch_assoc($res2)) {
	...
	$res = mysql_query("SELECT g.date, g.latitude, g.longitude, g.speed, g.address FROM gprmc g WHERE gpsSignalIndicator = 'F' and imei = ". $imei ." ORDER BY id DESC LIMIT 1");
	$line_coordinates = "";
	$ballons = "";
	...
	while($data = mysql_fetch_assoc($res)) {
		...
		if ($address == null or $address == "") {
			...
			$address = reverse_geocode($longitudeDecimalDegrees,$longitudeDecimalDegrees);

		
			if (!mysql_query("UPDATE gprmc set address = '". utf8_decode($address) ."', date = date where id = $idRota")) {
		...
		if ($loop == 0) {
			...
			<Placemark>
				...
				<styleUrl>#highlightPlacemarkGreen'.$imei.'</styleUrl>
				<description>
					<![CDATA[
						<font face="Arial" style="font-size:11px">
							...
							<a href=./mapa.php?entries=20&imei='.$imei.'>Seguir no Mapa</a>
						</font>
					]]>
				</description>
				<Point>
	...
	$res = mysql_query("SELECT id, extended_info, time, latitude, longitude, speed, address FROM positions WHERE device_id = '". $idoutros. "' ORDER BY id DESC LIMIT 1");
		while($data = mysql_fetch_assoc($res)) 
	{
		...
		if ($address == null or $address == "") {
			...
			$address = reverse_geocode($longitude,$longitude);
		
			if (!mysql_query("UPDATE positions set address = '". utf8_decode($address) ."' where id = $idRota")) {
				//die('Error: ' . mysql_error());
			}
		}
			...
			<Placemark>
				...
				<description>
					<![CDATA[
						<font face="Arial" style="font-size:11px">
							End.: '. $address .' <br/>
							Veloc. : '.floor($speed).'Km/h - Data : '.date('d/m/Y H:i:s', strtotime($data['time'])).' <br/>
