					...
					*/
					...
					if (count($parts) > 1) 
					{
					  ...
					  $gpsSignalIndicator         = mysql_real_escape_string($parts[4]);
					    ...
					    $datat = "20" . substr($trackerdate, 0, 2) . "/"  . substr($trackerdate, 2, 2) . "/"  . substr($trackerdate, 4, 2) . " "  . substr($trackerdate, 6, 2) . ":"  . substr($trackerdate, 8, 2) . ":"  . substr($trackerdate, 10, 2) ;
					  if (strlen($trackerdate) < 11)
					  {
					  
					  $seg = date("s");
					  $datat = "20" . substr($trackerdate, 0, 2) . "/"  . substr($trackerdate, 2, 2) . "/"  . substr($trackerdate, 4, 2) . " "  . substr($trackerdate, 6, 2) . ":"  . substr($trackerdate, 8, 2) . ":" . $seg ;
					  }
					...
					$longitudeDecimalDegrees = $longitudeDecimalDegrees * -1;
					            ...
					            if ($infotext == 'OBD') 	{	
								  ...
								  if (strlen($trackerdate) < 11)
								  {
								  $datat = "20" . substr($trackerdate, 0, 2) . "/"  . substr($trackerdate, 2, 2) . "/"  . substr($trackerdate, 4, 2) . " "  . substr($trackerdate, 6, 2) . ":"  . substr($trackerdate, 8, 2) . ":00" ;
								  }
								
										
									  $acumk2 = mysql_real_escape_string($parts[2]);
									  $acumkm = mysql_real_escape_string($parts[3]);
									  $nivelcomb = mysql_real_escape_string($parts[4]);
									  $mediacomb = mysql_real_escape_string($parts[5]);
									  $tempdir = mysql_real_escape_string($parts[6]);
									  $speed = mysql_real_escape_string($parts[7]);
									  $cargaen = mysql_real_escape_string($parts[8]);
									  $tempagua = mysql_real_escape_string($parts[9]);
									  $percacel = mysql_real_escape_string($parts[10]);
									  $velmotor = mysql_real_escape_string($parts[11]);
									  $tensbateria = mysql_real_escape_string($parts[12]);
									  $diag1 = mysql_real_escape_string($parts[13]);	
									  $diag2 = mysql_real_escape_string($parts[14]);	
									  $diag3 = mysql_real_escape_string($parts[15]);								  
									  $diag4 = mysql_real_escape_string($parts[16]);
									  
									  
									  
									
									  echo '$acumkm:' . $acumkm ;
									  echo '$nivelcomb:' .	  $nivelcomb; 
									  echo '$mediacomb:' .  $mediacomb ;
									  echo '$tempdir:' . $tempdir ;
									  echo '$speed:'. $speed ;
									  echo '$cargaen:' . $cargaen ;
									  echo '$tempagua:'.$tempagua;
									  echo '$percacel:' .  $percacel ;
									  echo '$velmotor:' .$velmotor ;
									  echo '$tensbateria:' .$tensbateria ;
									  echo '$diag1:' .$diag1 ;
									  echo '$diag2:' .$diag2 ;
									  echo '$diag3:' .$diag3 ;						  
									  echo '$diag4:' .$diag4 ;
						...
						$result = 	mysql_query("INSERT INTO obd (data, infotext, imei, kmacul, combus, medcomb, tempdir, vel, batt, tempag, percacel, rtmotor, tnsbatt, diagcar1, diagcar2, diagcar3, diagcar4 ) 
						                       VALUES ('$datat', '$infotext' , '$imei' , '$acumkm', '$nivelcomb', '$mediacomb', '$tempdir' , '$speed', '$cargaen', '$tempagua' , '$percacel', '$velmotor', '$tensbateria', '$diag1', '$diag2', '$diag3', '$diag4' )", $cnx);
					  ...
					  if ($gpsSignalIndicator != 'L')
					  {
						...
						mysql_query("INSERT INTO gprmc (date, imei, phone, satelliteFixStatus, latitude, longitude, speed, infotext, gpsSignalIndicator) VALUES ('$datat', '$imei', '$phone', '$satelliteFixStatus', '$latitudeDecimalDegrees', '$longitudeDecimalDegrees', '$speed', '$infotext', '$gpsSignalIndicator')", $cnx);
					  }
					  else 
					  {
